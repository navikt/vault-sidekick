		...
		for {
			...
			case x := <-retrieveChannel:
				...
				if x.resource.maxRetries > 0 && x.resource.retries > x.resource.maxRetries {
					glog.V(4).Infof("skipping resource %s as it's failed %d/%d times", x.resource.path, x.resource.retries, x.resource.maxRetries+1)
					break
				}
...
func newVaultClient(opts *config) (*api.Client, error) {
	...
	if len(opts.vaultTokenOutputFile) > 0 {
		...
		glog.Infof("token saved to %s", opts.vaultTokenOutputFile)
	}
